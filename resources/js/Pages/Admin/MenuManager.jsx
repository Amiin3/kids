...
export default function MenuManager({ auth, adminMenus, appMenus, resellerMenus = [], h2hMenus = [], webSettings }) {
    ...
    const saveWebSettings = (e) => {
        ...
        router.post('/admin/settings/save', formData, {
            onSuccess: () => Swal.fire({ icon: 'success', title: 'Tersimpan!', showConfirmButton: false, timer: 1000 }),
        });
    };

    // FIX TAMPILAN LOGO (Object-Contain + Padding agar tidak terpotong tepi)
    const renderIcon = (iconString) => {
        ...
        if (iconString.includes('/') || iconString.includes('http')) {
            return <img src={iconString} alt="icon" className="w-full h-full object-cover" />;
        }
        return <i className={`fa-solid ${iconString} text-xl`}></i>;
    };

    const renderMenuGrid = (menus, title, colorClass) => (
        <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
                <span className={`w-2 h-2 rounded-full ${colorClass} animate-pulse`}></span>
                <h3 className="font-black text-sm uppercase tracking-widest text-slate-400">{title}</h3>
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 bg-slate-800/30 p-5 rounded-3xl border border-slate-800">
                {menus.map((menu) => (
                    <div key={menu.id} onClick={() => handleEditMenu(menu)} className={`flex flex-col items-center group cursor-pointer bg-slate-800/50 p-4 rounded-2xl border border-slate-700/40 hover:border-indigo-500/80 hover:bg-slate-800 transition-all transform hover:-translate-y-1 ${menu.is_active === 0 ? 'opacity-40 grayscale' : ''}`}>
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-md relative overflow-hidden mb-2 ${menu.bg || 'bg-slate-700'}`}>
                            {renderIcon(menu.icon)}
                        </div>
                        <span className="text-[10px] font-black text-slate-300 text-center leading-tight tracking-tight group-hover:text-white line-clamp-1">
                            {menu.is_active === 0 && '👁️‍🗨️ '} {menu.name}
                        </span>
                    </div>
                ))}
            </div>
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="CMS Navigasi MILASTORE" />
            <div className="p-6 bg-slate-900 min-h-screen text-white font-sans">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-800 pb-6">
                    <div>
                        <h2 className="font-black text-2xl tracking-tighter uppercase text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400">Pusat Kendali Navigasi MILASTORE</h2>
                        <p className="text-xs text-slate-400 font-bold mt-1">Sembunyikan menu, atur warna, dan kelola logo layanan dengan 1 klik.</p>
                    </div>
                ...
                <div className="flex flex-wrap gap-2 mb-8 bg-slate-800/60 p-2 rounded-2xl border border-slate-700/50 w-max">
                    <button onClick={() => setActiveTab('all')} className={`px-4 py-2 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'all' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>✨ Semua</button>
                    <button onClick={() => setActiveTab('app')} className={`px-4 py-2 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'app' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>📱 User ({appMenus.length})</button>
                    <button onClick={() => setActiveTab('reseller')} className={`px-4 py-2 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'reseller' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>💼 Reseller ({resellerMenus.length})</button>
                    <button onClick={() => setActiveTab('h2h')} className={`px-4 py-2 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'h2h' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>🔗 H2H ({h2hMenus.length})</button>
                    <button onClick={() => setActiveTab('admin')} className={`px-4 py-2 rounded-xl font-black text-xs uppercase transition-all ${activeTab === 'admin' ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>🛡️ Admin ({adminMenus.length})</button>
                </div>
