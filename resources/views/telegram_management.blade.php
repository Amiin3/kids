...
<head>
    ...
    <title>MILASTORE MANAGEMENT</title>
...
<body class="antialiased min-h-screen pb-24" x-data="management()">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-sm font-extrabold tracking-tight text-white flex items-center gap-1.5">
                    MILASTORE <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30">MANAGEMENT</span>
                </h1>
    ...
    <main class="p-4 max-w-md mx-auto space-y-4">
        ...
        <div x-show="activeTab === 'dashboard'" x-transition.opacity.duration.300ms class="space-y-4">
            ...
            <div class="glass-card rounded-2xl p-4 space-y-3 mt-4">
                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Management Transaksi</h3>
                <div class="grid grid-cols-2 gap-2.5">
                    <button @click="alert('Fitur Approve Deposit Segera Aktif')" class="py-2.5 px-3 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/30 text-purple-400 text-xs font-bold flex items-center justify-center gap-2 transition active:scale-95">
                        <i class="fa-solid fa-money-bill-transfer"></i> Cek Deposit
                    </button>
                    <button @click="alert('Fitur Monitoring Transaksi Segera Aktif')" class="py-2.5 px-3 rounded-xl bg-orange-600/20 hover:bg-orange-600/30 border border-orange-500/30 text-orange-400 text-xs font-bold flex items-center justify-center gap-2 transition active:scale-95">
                        <i class="fa-solid fa-clock-rotate-left"></i> Trx Pending
                    </button>
                </div>
            </div>
        <!-- TAB 2: SECURITY -->
        <div x-show="activeTab === 'security'" x-transition.opacity.duration.300ms class="space-y-4" style="display: none;">
            ...
            <div class="glass-card rounded-2xl p-4">
                ...
                <div class="relative flex items-center">
                    ...
                    <input type="text" x-model="searchIp" placeholder="Cari IP (contoh: 192.168...)" class="w-full bg-slate-900/80 border border-slate-700/80 rounded-xl pl-8 pr-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono">
                </div>
            </div>
                    <span class="text-[10px] text-slate-500" x-text="`${data.security.banned_list.length} IP`"></span>
                </div>
                <div class="space-y-2 max-h-60 overflow-y-auto pr-1">
                    <template x-for="item in data.security.banned_list.filter(i => i.ip.includes(searchIp))" :key="item.ip">
        ...
        function management() {
            return {
                ...
                async fetchData() {
                    this.loading = true;
                    try {
                        const res = await fetch("/telegram/management/data");
                        const json = await res.json();
                        if(json.error) {
                            alert("⚠️ Gagal Memuat Data (Database Error): " + json.error);
                        } else {
                            if(json.security) this.data.security = json.security;
                            if(json.finance) this.data.finance = json.finance;
                            if(json.system) this.data.system = json.system;
                        }
                    } catch (e) {
                        alert("Koneksi terputus. Silakan klik tombol Refresh.");
                ...
                async runScanAction(autoFix) {
                    ...
                    try {
                        const res = await fetch('/telegram/management/scan', {
                            method: 'POST',
                ...
                async unblock(ip) {
                    ...
                    try {
                        await fetch('/telegram/management/unblock', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': this.csrf },
                ...
                async clearCacheAction() {
                    this.loading = true;
                    try {
                        await fetch('/telegram/management/clear-cache', { 
                            method: 'POST',
                ...
                async runOsint() {
                    ...
                    this.loading = true;
                    try {
                        const res = await fetch('/telegram/management/osint', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': this.csrf },
