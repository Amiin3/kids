...
class OrderPulsaController extends Controller
{
    ...
    public function index() {
        // 1. Ambil Pulsa Reguler dari Digiflazz
        $digi = DB::table('layanan')->where('status', 'active')->where(function($q) {
            $q->where('tipe', 'LIKE', '%PULSA%')
              ->orWhere('nama_layanan', 'LIKE', '%PULSA%');
        })->get()->map(function($item) { $item->server = 'DIGIFLAZZ'; return $item; });
        
        // 2. Ambil Pulsa Promo dari Okeconnect
        ...
        // Gabungkan dan Urutkan Harga dari Termurah
        $products = collect($digi)->merge($oke)->sortBy('harga_jual')->values();
        
        // Kategori Pintar
        $grouped = [
            'Telkomsel' => [], 'Indosat' => [], 'Tri' => [], 'XL' => [],
            'Axis' => [], 'Smartfren' => [], 'By.U' => [], 'Lainnya' => []
        ];

        foreach ($products as $p) {
            $name = strtoupper($p->nama_layanan ?? '');
            $brand = strtoupper(isset($p->brand) ? $p->brand : '');
            
            if (str_contains($name, 'BY.U') || str_contains($brand, 'BY.U')) {
                $grouped['By.U'][] = $p;
            } elseif (str_contains($name, 'TELKOMSEL') || str_contains($brand, 'TELKOMSEL') || str_contains($name, 'TSEL')) {
                $grouped['Telkomsel'][] = $p;
            } elseif (str_contains($name, 'INDOSAT') || str_contains($brand, 'INDOSAT') || str_contains($name, 'ISAT')) {
                $grouped['Indosat'][] = $p;
            } elseif (str_contains($name, 'TRI') || str_contains($brand, 'TRI') || str_contains($name, 'THREE')) {
                $grouped['Tri'][] = $p;
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            return back()->with('success', 'Pembelian Pulsa Berhasil Diproses!');
