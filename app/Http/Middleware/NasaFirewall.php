...
class NasaFirewall
{
    ...
    {
        // 1. MENDAPATKAN REAL IP SEJAK AWAL
        $ip = $request->header('CF-Connecting-IP', $request->header('X-Forwarded-For', $request->ip()));
        if (str_contains($ip, ',')) {
            $ip = trim(explode(',', $ip)[0]);
        }

        // 🌟 2. JALUR VVIP MUTLAK (ANTI SENJATA MAKAN TUAN)
        // Bypass semua penjagaan jika IP dari Server Sendiri atau User-Agent adalah WarEngine
        $rawUserAgent = $request->header('User-Agent', '');
        $whitelistIPs = ['159.223.185.13', '127.0.0.1'];
        
        if (in_array($ip, $whitelistIPs) || stripos($rawUserAgent, 'warengine') !== false) {
            return $next($request); // 🚀 BEBAS HAMBATAN 100%!
        }
        ...
        // 6. STRICT HTTP METHOD & BUFFER OVERFLOW PROTECTION
        ...
        // 7. ANTI BACKDOOR / MALICIOUS FILE UPLOAD SCANNER
    ...
    private function punishAndAlert(Request $request, $ip, $reason)
    {
        ...
        $locInfo = [
            'country' => 'Tidak Diketahui', 'region' => 'Tidak Diketahui', 'city' => 'Tidak Diketahui',
            'zip' => '-', 'timezone' => 'Tidak Diketahui', 'isp' => 'Tidak Diketahui',
            'asn' => 'Tidak Diketahui', 'coords' => 'Tidak Diketahui', 'connection_type' => '❌ Bersih (Residential/Mobile)'
        ];
        ...
        $mapsLink = ($locInfo['coords'] !== 'Tidak Diketahui') ? "https://www.google.com/maps/search/?api=1&query=" . urlencode($locInfo['coords']) : "Tidak Tersedia";
        ...
        DB::table('security_logs')->insert([
            'ip' => $ip, 'url' => $url, 'pattern' => "Threat: $reason | OS: {$deviceInfo['os']} | Loc: {$locInfo['city']}, {$locInfo['country']}",
    ...
    {
        $result = ['type' => 'Desktop / PC', 'os' => 'Tidak Diketahui', 'browser' => 'Tidak Diketahui / Custom Script'];
